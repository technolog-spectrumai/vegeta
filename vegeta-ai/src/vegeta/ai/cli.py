"""``vegeta ai ...``: propose and accept design changes from the command line."""
...
def main(argv=None, prog: str = "vegeta ai") -> int:
    ap = argparse.ArgumentParser(prog=prog, description="Claude design copilot for Dedalus designs")
    ap.add_argument("--model", default="claude-opus-5")
    ap.add_argument("--effort", default="high", choices=["low", "medium", "high", "xhigh", "max"])
    ap.add_argument("--api-key", default=None, help="default: ANTHROPIC_API_KEY")
    ap.add_argument("--max-tokens", type=int, default=16000)
    sub = ap.add_subparsers(dest="cmd", required=True)
    p = sub.add_parser("propose", help="ask for a change; prints the validated proposal, writes nothing")
    ...
    a = sub.add_parser("accept", help="apply a saved proposal to the design file (backup kept)")
    a.add_argument("design")
    a.add_argument("proposal_json")
    args = ap.parse_args(argv)
    try:
        if args.cmd == "propose":
            proposer = ClaudeProposer(ClaudeConfig(api_key=args.api_key, model=args.model, effort=args.effort,
                                                   max_tokens=args.max_tokens))
            session = DesignSession(args.design, proposer, parameters=_parse_params(args.param), notes=args.notes)
