...
def write_inp(path: Path, mesh: MeshData, material, supports, loads) -> dict:
    """Write a linear static CalculiX deck. Returns bookkeeping (applied loads, sets)."""
    cmap = mesh.coord_map()
    lines = ["*HEADING", "Talos linear static analysis", "*NODE, NSET=NALL"]
    ...
    used_regions = {s.region for s in supports} | {l.region for l in loads if hasattr(l, "region")}
    ...
    lines += ["*SOLID SECTION, ELSET=EALL, MATERIAL=MAT", "*STEP", "*STATIC"]
